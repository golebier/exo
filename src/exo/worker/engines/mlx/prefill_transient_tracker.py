"""Per-model EWMA of bytes-per-prefill-token.

Ported from oMLX's ``omlx/prefill_transient_tracker.py``. Used by the
per-chunk prefill guard (Phase 2 of doc 11) and the preflight admission
estimate: when current memory enters the caution zone, the next chunk is
sized/aborted so its predicted transient stays under the remaining headroom.

One tracker per loaded model (owned by ``KVPrefixCache``), distinct from the
global prefill-progress events that feed the dashboard. The EWMA is updated
post-chunk from memory-footprint deltas. The first chunk has no measurement
yet — callers fall back to the static estimate
(``estimate_prefill_peak_bytes``) until samples > 0.
"""
...
class PrefillTransientTracker:
    ...
    Updated post-chunk from memory-footprint deltas. The first chunk has no
    ...
    # the last-delta/EWMA terms of ``predicted_chunk_transient``.
    ...
    # blend. See oMLX's docstring for the incident (2026-07-29,
    # Qwen3.6-35B-A3B) that motivated 8x.
    ...
    def clear_reclaim(self) -> None:
        ...
        EWMA gates skip (sub-floor tails) — the footprint has grown back, so
        keeping the charge would double count against the guard's gates.
        """
    ...
    def update(
        ...
        if self._samples == 0:
            ...
            # the running rate is more likely a noisy footprint delta than
            # a real per-token cost jump. last_delta_bytes/last_n_tokens
            # below still record it raw for diagnostics.
            logger.debug(
