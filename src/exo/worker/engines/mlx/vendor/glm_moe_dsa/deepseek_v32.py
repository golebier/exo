...
"""Patched DeepSeek-V3.2 model for GLM-5.2.

Ported from jundot/omlx ommlx/patches/glm_moe_dsa/deepseek_v32.py.
This version adds GLM-5.2-specific optimizations (fused indexer scores,
fused gate/up projections, weighted MoE sum) gated on model_type == "glm_moe_dsa".
The base DeepSeek-V3.2 model is reused from the installed mlx_lm package.
"""
...
def _use_load_fused_wk_weights_proj(args: object) -> bool:
    """Return True if the indexer's wk/weights_proj should be fused into one Q8 linear."""
    ...
    def resolved_spec(name: str) -> dict[str, Any] | None:
        spec = quantization.get(name, quantization)  # type: ignore[arg-type, index]
        return spec if isinstance(spec, dict) else {}

    def is_indexer_q8(name: str) -> bool:
    ...
    # Preserved MTP checkpoints use runtime mtp.* paths.
    for key in quantization:  # type: ignore[iterator]
    ...
    if len(q8_paths) != len(paths):
        offenders = [
            (
                path,
                resolved_spec(path).get("bits") if resolved_spec(path) else None,
                resolved_spec(path).get("group_size") if resolved_spec(path) else None,
                resolved_spec(path).get("mode", "affine")
                if resolved_spec(path)
                else None,
            )
            for path in paths
...
def _dequant_mla_proj_mode(args: object) -> str:
    return "0"


def _use_glm_moe_fused_gate_up(args: object) -> bool:
    return getattr(args, "model_type", None) == "glm_moe_dsa"


def _use_glm_shared_fused_gate_up(args: object) -> bool:
    return False


def _use_glm_moe_weighted_sum(args: object) -> bool:
    return getattr(args, "model_type", None) == "glm_moe_dsa"


def _use_glm_moe_inverse_scatter(args: object) -> bool:
    return getattr(args, "model_type", None) == "glm_moe_dsa"
...
class ModelArgs(BaseModelArgs):
    ...
    rope_scaling: Dict = None  # type: ignore[assignment]
    attention_bias: bool = False
    indexer_rope_interleave: bool = False


# Decode-shaped multi-row forwards must not enter the prefill-shaped fused
# scores kernel. 16 mirrors the sdpa256/fa256 prefill-routing floors.
...
class Indexer(nn.Module):
    ...
    def _fused_wk_weights_proj(self, x: mx.array) -> tuple[mx.array, mx.array] | None:
    ...
    def __call__(
        ...
        def causal_prefix_topk(prefix_rows: int) -> mx.array:
            offset_val = k.shape[2] - s
            ...
            lengths = mx.arange(prefix_rows, dtype=mx.uint32).reshape(
                1, 1, prefix_rows, 1
            ) + mx.array(offset_val + 1, dtype=mx.uint32)
        ...
        def finish_topk_indices(
            indices: mx.array | None, prefix_rows: int = 0
        ) -> mx.array | tuple[mx.array | None, None, int]:
        ...
        def select_topk(
            ...
            prefix_rows: int = 0,
            causal_valid_prefix: Optional[bool] = None,
        ) -> mx.array | tuple[mx.array | None, None, int]:
        ...
        if s >= _FUSED_SCORES_MIN_S and (mask is None or fuse_causal_mask):
            ...
            chunked_topk: list[mx.array | None] | None = None
            if chunk_mb > 0 and use_fast_topk and score_q.shape[2] > 64:
                ...
                if chunk_rows < score_q.shape[2]:
                    chunk_indices: list[mx.array | None] = []
                    ...
                    if chunk_indices:
                        chunked_topk = mx.concatenate(chunk_indices, axis=2)  # type: ignore[arg-type]
        ...
        head_scores: mx.array | None = None
...
class DeepseekV32MoE(nn.Module):
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        use_weighted_sum = (
            ...
            and inds.size >= 64
            and glm_fast.has("glm_moe_weighted_sum")
        )
        y = self.switch_mlp(
            x,
            inds,
...
class Model(nn.Module):
    ...
    def sanitize(self, weights: dict[str, mx.array]) -> dict[str, mx.array]:
        ...
        if (dequant_embed_q or dequant_unembed_out) and isinstance(
            ...
            def dequant_mla_weight(path: str, input_dims: int) -> None:
                ...
                if scales is None:
                    return
                spec = self.args.quantization.get(path, {})  # type: ignore[arg-type, index]
                bits = spec.get("bits")
                if bits is None:
            ...
            for l in range(self.args.num_hidden_layers):
                ...
                if dequant_embed_q:
                    ...
                    self.args.quantization.pop(f"{prefix}.embed_q", None)  # type: ignore[attr-defined]
                if dequant_unembed_out:
                    dequant_mla_weight(f"{prefix}.unembed_out", self.args.kv_lora_rank)
                    self.args.quantization.pop(f"{prefix}.unembed_out", None)  # type: ignore[attr-defined]
        ...
        for l in range(self.args.num_hidden_layers):
                ...
                for k in ["weight", "scales", "biases"]:
                    if f"{prefix}.mlp.experts.0.{m}.{k}" in weights:
                        to_join = [
                            ...
                            for e in range(self.args.n_routed_experts)  # type: ignore[arg-type]
                        ]
            ...
            if _use_glm_moe_fused_gate_up(self.args):
                ...
                # Remap per-layer quantization specs so the fused
                # ``gate_up_proj`` module picks up the correct bits/group_size
                # instead of falling back to the top-level defaults during
                # ``nn.quantize`` (the config only has ``gate_proj``/``up_proj``
                # entries, which no longer match any module path after fusion).
                ...
                if isinstance(quant_cfg, dict):
                    ...
                    # gate_proj and up_proj share the same quantization spec;
                    # fall back to either side (or the up_spec if gate missing).
                    fused_spec = gate_spec or up_spec
