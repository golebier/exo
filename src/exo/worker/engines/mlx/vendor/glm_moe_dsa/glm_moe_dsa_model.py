...
"""GLM-5.2 glm_moe_dsa model implementation.

Ported from jundot/omlx ommlx/patches/glm_moe_dsa/glm_moe_dsa_model.py.

The key GLM-5.2-specific changes from the base DeepSeek-V3.2 model:
  - `ModelArgs` includes all GLM-5.2 fields (indexer_types, index_topk_freq,
    index_skip_topk_offset, quantization, quantization_config) and derives
    indexer_types from the freq/offset pattern when not provided.
  - `GlmMoeDsaAttention` sets `self.indexer = None` for shared layers and
    uses `prev_topk_indices` from the full layer in the group.
  - `GlmMoeDsaDecoderLayer` threads `prev_topk_indices` between layers.
  ...
  - `Model.make_cache()` gives shared layers a single KVCache (no indexer
    cache) since they don't run their own indexer.
"""
...
# L = depth+1 <= 8 rows). Below this floor the native sparse kernel would
# be slower than the plain per-row top-k gather path.
_ABSORBED_DECODE_MAX_L = 8


def _parse_topk_state(
    topk_state: mx.array | tuple[mx.array | None, None, int] | None,
) -> tuple[mx.array | None, int]:
    """Extract topk_indices and prefix_rows from the indexer's return state."""
...
def _apply_sparse_topk_mask(
    ...
    *,
    ...
    """Apply the sparse top-k mask to the attention mask."""
...
class GlmMoeDsaAttention(DeepseekV32Attention):
    """GLM-5.2 attention with indexer sharing.

    Shared layers (indexer_types[i] == "shared") do not own an indexer and
    instead reuse the top-k indices computed by the nearest full layer in
    their group.
    """
    ...
    def __call__(
    ...
    ) -> tuple[mx.array, mx.array | tuple[mx.array | None, None, int] | None]:
        B, L, D = x.shape
        ...
        if (
            ...
            # Mask gathered keys past each row's own absolute position.
            row_pos = mx.arange(
        ...
        native_sparse_mla_shape = (
            topk_indices is not None
            and self.num_heads in (32, 64)  # 32 = tensor-sharded half of 64 MLA heads
            and q_pe.shape[-1] == 64
...
class GlmMoeDsaDecoderLayer(DeepseekV32DecoderLayer):
    """Decoder layer that threads prev_topk_indices to the next layer."""
...
class GlmMoeDsaModel(DeepseekV32Model):
    """Model that threads prev_topk_indices through the layer loop."""
...
class Model(DSV32Model):
    """Top-level model wrapping the GLM-5.2 model with indexer sharing."""
    ...
    def sanitize(self, weights: dict[str, mx.array]) -> dict[str, mx.array]:
        """Remove indexer weights for shared layers.

        Shared layers run no indexer, so their indexer weights should not be
        loaded (they would be random/uninitialized). This drops them from the
        weight dict so the model loads cleanly.
        """
