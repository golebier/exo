...
"""Sparse MLA attention helpers for GLM-5.2.

These helpers mirror the oMLX GLM-5.2 sparse-attention path. They use native
Metal kernels when available (via oMLX's custom_kernels) and fall back to
returning None, which causes the caller to use the standard scaled_dot_product
attention path with the sparse top-k mask applied.

Ported from jundot/omlx ommlx/patches/glm_moe_dsa/sparse_mla.py.
"""
...
def _make_topk_indices_to_block_masks_kernel() -> object | None:
    """Build the Metal kernel for top-k indices to block masks, if available."""
...
def exact_block_token_attention(
    ...
    *,
    ...
    if (
        not glm_fast.has("glm_dsa_exact_block_attention")
        or q.ndim != 4
        or k.ndim != 4
        or v.ndim != 4
...
@lru_cache(maxsize=None)
def _make_index_score_reduce_kernel() -> object | None:
    """Build the fused index score reduce Metal kernel, if available."""
    if not mx.metal.is_available():
        return None
...
def fused_indexer_scores(
    ...
    *,
    ...
    if (
        not glm_fast.has("dsa_indexer_scores")
        or queries.ndim != 4
        or keys.ndim != 4
        or weights.ndim != 3
...
def sparse_mla_attention(
    ...
    *,
    ...
    if not glm_fast.has("glm_dsa_sparse_mla_attention"):
        return None
...
def q8_vup_flat(
    ...
    *,
    ...
    if (
        not glm_fast.has("glm_dsa_q8_vup_flat")
        or x.ndim != 4
        or x.shape[1] != 64
        or x.shape[-1] != 512
    ...
    biases = unembed_out.get("biases") if hasattr(unembed_out, "get") else None  # type: ignore[attr-defined]
    if biases is None:
        return None
    weight = unembed_out["weight"]  # type: ignore[index]
    scales = unembed_out["scales"]  # type: ignore[index]
