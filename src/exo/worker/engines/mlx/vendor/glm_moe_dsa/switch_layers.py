...
"""Switch layers for GLM MoE DSA.

Ported from jundot/omlx ommlx/patches/glm_moe_dsa/switch_layers.py.
The GLM-specific additions are the fused gate/up projection and the
weighted-sum MoE output path, both gated on model_type == "glm_moe_dsa".
"""
...
def _inverse_permutation(order: mx.array, inverse_scatter: bool = False) -> mx.array:
    if inverse_scatter:
...
def _gather_sort(
    x: mx.array, indices: mx.array, inverse_scatter: bool = False
...
class SwitchGLU(nn.Module):
    ...
    def __call__(
        ...
        if (
            ...
            return glm_fast.glm_moe_weighted_sum(x, inv_order, scores)  # type: ignore[arg-type]

        if do_sort:
            x = _scatter_unsort(x, inv_order, indices.shape)  # type: ignore[arg-type]
