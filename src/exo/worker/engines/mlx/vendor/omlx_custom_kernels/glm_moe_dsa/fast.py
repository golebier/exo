...
Vendored verbatim from oMLX ``omlx/custom_kernels/glm_moe_dsa/fast.py``
(commit 1f1aff3018c097bcbeafca9a483e58d04dee38ba) so EXO can build and load
the native ``_ext`` nanobind extension without depending on the ``omlx``
package at runtime. The body is intentionally kept byte-identical to upstream
to make future syncs trivial; only this attribution header was added.

EXO's ``exo.worker.engines.mlx.vendor.glm_moe_dsa.kernels`` dispatch imports
this module as ``_native_fast`` and gates every call behind
``has_symbol(name)``, so when the ``_ext`` extension is not built the GLM
symbols report unavailable and the model falls back to the standard
attention path — identical to EXO's pre-kernel behavior.
"""
